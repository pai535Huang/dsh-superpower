...
import { createSkillProvider } from './skills.mjs'
...
export function apply(ctx) {
  ...
  try {
    ...
    const provider = createSkillProvider(skillsRoot)
    ctx.skills.registerProvider(provider)
    ctx.logger?.info?.(`dsh-superpower: registered a skill provider (skills auto-discovered from ${skillsRoot})`)
  } catch (error) {
    ctx.logger?.warn?.(`dsh-superpower: skill provider registration failed: ${String(error?.message ?? error)}`)
  }
